...
example, altlocs `A` and `B` may describe two modeled positions of one
side chain. Each atom record also has an **occupancy**, normally a
number from 0 to 1 that estimates the fraction of the modeled structure
represented at that position. Thus, occupancies of 0.70 and 0.30 suggest
that the first position is the more prevalent of the two. Occupancy is
not an atom-quality score; in this context it is used as an estimate of
the relative population of alternate positions.

Feeding every alternate position to the model would incorrectly
duplicate atoms, while choosing whichever record appears first would
make the result depend on file ordering. The pipeline therefore groups
records that describe the same physical atom and selects exactly one
position deterministically. It first chooses the record with the highest
occupancy. If two records have the same occupancy, it prefers a blank
altloc, which means that the record was not assigned an
alternate-position label. If a further tie remains, it prefers altloc
`A`. Here, `A` is simply the one-character label conventionally assigned
by a structure depositor to the first alternate conformation; it is not
an element symbol, an amino-acid code, or evidence that the position is
scientifically better. Remaining ties are resolved by the lexically
smallest altloc label (`B` before `C`, for example), then by original
source order. Discarded alternatives remain auditable through the
